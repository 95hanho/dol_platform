...
export default function Join() {
  ...
  // 텍스트 상태 표시줄
  const [loginIdAlert, setLoginIdAlert] = useState(0);
  ...
  // 아이디 알람text
  const loginIdAlertText = () => {
    if (loginIdAlert === 1) return "사용 가능한 아이디입니다.";
    else if (loginIdAlert === 2) return "6~15자 영문숫자 조합. 숫자시작안됨.";
    else if (loginIdAlert === 3) return "잘못된 아이디 형식입니다.";
    else if (loginIdAlert === 4) return "아이디 중복확인을 해주세요.";
    else if (loginIdAlert === 5) return "이미 존재하는 아이디입니다.";
  };
  // 이메일url에 따른
  const changeEmailKinds = (e) => {
    setEmailKinds(e.target.value);
    if (e.target.value === "직접입력") setEmailAddressReadOnly(false);
  ...
  // 아이디 중복확인
  const idDuplCheckBefore = (e) => {
    console.log("idDuplCheckBefore");
    e.preventDefault();
    const idRegExp = /^(?![0-9])[a-zA-Z0-9]{6,15}$/;
    if (!idRegExp.test(user.loginId)) setLoginIdAlert(3);
    else {
      if (loding) return;
      dispatch({ type: "loding/lodingOn" });
      idDuplCheck({ loginId: user.loginId })
        .then(({ data }) => {
          console.log(data);
          setLoginIdAlert(1);
          setIdDupl(true);
  ...
  const pwdSameCheck = (e) => {
    ...
    if (e.target.value && user.password === e.target.value) setPwdCheckAlert(1);
  ...
  const authCheckBefore = (e) => {
    ...
    if (authIng && authReadOnly) setAuthAlert(4);
    else if (!authRegExp.test(user.emailAuth)) setAuthAlert(5);
  ...
  // 회원가입 전 체크
  const joinBefore = async (e) => {
    console.log("joinBefore");
    e.preventDefault();
    console.log(user);
    ...
    if (user.nickName.length <= 1) setNickNameAlert(true);
    ...
    // 포커스하기
    if (!idDupl) {
      refObj.loginId.current.focus();
      setLoginIdAlert(4);
    } else if (!pwdRegExp.test(user.password)) refObj.password.current.focus();
    else if (user.password !== user.pwdCheck) refObj.pwdCheck.current.focus();
    else if (user.membName.length <= 1) refObj.membName.current.focus();
    else if (user.nickName.length <= 1) refObj.nickName.current.focus();
  ...
  useEffect(() => {
    setUser({
      ...user,
      emailAddress: emailKinds === "직접입력" ? "" : emailKinds,
    });
  }, [emailKinds]);
  useEffect(() => {
    getDepts().then(({ data }) => {
  ...
  return (
    <>
      <Header noLogin={true} backUse={true} title={"회원가입"} />
      <main id="login-main" className="join-main">
        <div className="login-wrap">
          <section className="login-section">
            <form onSubmit={joinBefore}>
              <p>아이디</p>
              <div className="inputdiv">
                <div className="btn-exist-left">
                  <input
                    ...
                    ref={refObj.loginId}
                    onFocus={() => {
                      if (loginIdAlert !== 1) setLoginIdAlert(2);
                    }}
                    onBlur={() => {
                      if (loginIdAlert !== 1) setLoginIdAlert(0);
                    }}
                  />
                  <button
                    type="button"
              ...
              <p
                className={loginIdAlert === 1 ? "c_green" : "c_red"}
                style={{ display: loginIdAlert > 0 ? "block" : "none" }}
              >
                * {loginIdAlertText()}
              </p>
              <p>비밀번호</p>
              <div className="inputdiv">
                <input
                  ...
                  ref={refObj.password}
                  autoComplete="false"
                  onFocus={() => setPasswordAlert(true)}
                  onBlur={() => setPasswordAlert(false)}
                />
                <button
                  type="button"
                  tabIndex="-1"
                  className="clear-btn"
                  onClick={(e) => resetUser(e, "password")}
                >
                  <span className="material-icons"> highlight_off </span>
                </button>
              </div>
              <p className="c_red">
                {passwordAlert ? "* 8~16자 영문숫자특수문자 조합." : ""}
              </p>
              <p>비밀번호확인</p>
              <div className="inputdiv">
                <input
                ...
                <button
                  ...
                  tabIndex="-1"
                  className="clear-btn"
                  onClick={(e) => resetUser(e, "pwdCheck")}
                >
                  <span className="material-icons"> highlight_off </span>
                </button>
              </div>
              ...
              <p className="c_red">
                {nickNameAlert ? "* 닉네임을 2자 이상으로 입력해주세요." : ""}
              </p>
              ...
              <button className="loginBtn" onClick={joinBefore}>
                입력완료
              </button>
            </form>
          </section>
